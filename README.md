...
![]()
▶️[Go to preview video](https://youtu.be/1ZlV49kN6xw)
...
<기쁨 @joyfive>
- **Potal을 활용한 Modal 구현**
css의 `display : block;` `display : none;` 을 활용하여 개별 컴포넌트마다 state를 선언하고 노출여부를 결정해왔는데, potal을 활용함으로써 전역상태를 통해 여러 컴포넌트에서 활용하는 방안을 배웠습니다.<br>

- **Styled Components의 재사용성** 
기존에 styled components를 활용하면서 적극적인 재사용을 하는 방식이 익숙치 않았었는데, 재사용 가능한 만능버튼 컴포넌트를 만들어 임포트할때 너비값, 컬러 등을 커스텀해서 사용하는 스타일드 컴포넌트의 재사용성을 이해하게 되었습니다. <br>   

- **꼼꼼한 Validation** 
회원가입, 로그인 파트에서 프론트엔드의 꽃! 벨리데이션을 꼼꼼하게 적용해보았습니다. 아이디 사용가능여부, 정규식, 비밀번호 확인 등의 꼼꼼한 벨리데이션을 통해 사용자가 오류를 발생시키지 않도록 가이드를 제공합니다.<br>
  <br>
...
<기쁨 @joyfive>
- **시간관리 실패**
 실전 프로젝트에 들어가기에 앞서 미니 단위의 마지막 프로젝트이다보니, 팀원들 모두가 개개인의 담당파트를 끝까지 구현해보는 것을 목표로 진행하다보니 목표했던 배포 준비 시간을 넘기도록 개발작업이 이어졌습니다. 
<br>뒤늦게 배포를 진행할때가 되어서야 노드 모듈과 라이브러리의 충돌로 배포이슈가 있는 부분을 확인하여 결국 https가 아닌 로컬로 시연을 하게 되어 아쉬움이 남습니다. 
<br>실전 프로젝트에서는 일정 이슈가 발생하지않도록 데드라인을 넘기는 경우 과감하게 기능을 포기하고, 일정을 엄수할 수 있도록 적극적으로 일정관리를 해야할 것 같습니다.<br>
